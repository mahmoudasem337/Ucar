...
import { commonproblem as commonProblemOptions } from "../data/formOptions";
...
const VriceForm: React.FC<Props> = ({ formData, setFormData, onSubmit }) => {
  ...
  const handleInputChange = (
    ...
    const [commonProblemInput, setCommonProblemInput] = useState<string>("");
    const [filteredCommonProblems, setFilteredCommonProblems] = useState<string[]>([]);
  
// Autocomplete: handle typing
const handleProblemInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setCommonProblemInput(value);
    setFilteredCommonProblems(
      commonProblemOptions.filter((opt) =>
...
{/* Common Problem Autocomplete inside the existing form */}
<div className="input-group autocomplete-container">
          <label htmlFor="commonproblem">Common Problem:</label>
          <input
            id="commonproblem"
            name="commonproblem"
            type="text"
            ...
            autoComplete="off"
          />
          {filteredCommonProblems.length > 0 && (
            <ul className="autocomplete-options">
              {filteredCommonProblems.map((opt, idx) => (
                <li key={idx} onClick={() => handleSelectProblem(opt)}>
                  {opt}
                </li>
              ))}
            </ul>
